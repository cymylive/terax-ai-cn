...
export function GeneralSection() {
  ...
  // Sync close-to-tray state to Rust backend on mount
  useEffect(() => {
    const current = usePreferencesStore.getState().closeToTray;
    void invoke("set_close_to_tray", { enabled: current }).catch(() => undefined);
  }, []);
  ...
  const onToggleCloseToTray = async (next: boolean) => {
    try {
      await setCloseToTray(next);
      await invoke("set_close_to_tray", { enabled: next });
    } catch (e) {
      console.error("close to tray toggle failed", e);
    }
  };
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-2">
          ...
          >
            <Switch
              checked={closeToTray}
              onCheckedChange={(v) => void onToggleCloseToTray(v)}
            />
          </SettingRow>
          <SettingRow
